...
export class RemoteManager {
  ...
  public static run(room: Room) {
    ...
    if (Game.time % 100 === 0) {
      ...
      const remoteSources = _.reduce(
        Memory.roomStore[room.name].remoteRooms,
        (acc: { source: Source; hostile: boolean; pathLength: number }[], rRoom) => {
      ...
      remoteSources.map((t) => {
        if (
          !isSpawning &&
          !t.hostile &&
          _.filter(Game.creeps, (c) => c.memory.role === "harvesterShuttle" && c.memory.targetSource === t.source.id)
            .length < 2
        ) {
          const creepMemory = {
            ...CreepBase.baseMemory,
